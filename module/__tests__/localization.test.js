...
/**
 * These tests read the shipped templates and modules rather than any test
 * fixture. English text written straight into a template renders the same way
 * in every language, and a screen reader on a Portuguese client reads the
 * English out loud, so the checks below fail the build when such text reappears.
 */
 ...
 * Handlebars expressions, JS interpolations, and HTML tags. Anything with a
 * letter still in it is text that was typed in English and will stay English.
 */
...
/**
 * Read the values of one attribute out of a template.
 *
 * A regex cannot do this: a Handlebars expression inside the value carries its
 * own quotes ({{localize "DOCUMENT.Delete" type="Power"}}), and the first of
 * those would end the match early and hide whatever follows it.
 */
function attributeValues(contents, attribute) {
    const values = [];
    const opener = new RegExp(`${attribute}\s*=\s*['"]`, 'g');
...
describe('modules localize the text they show', () => {
    test('no notification is raised with a literal message', () => {
        const pattern = /ui\.notifications\.\w+\(\s*['"`]/g;
        const offenders = [];
    ...
    test.each(['title', 'content'])('no dialog %s is built from English text', (property) => {
        ...
        for (const file of modules) {
            ...
            for (const match of contents.matchAll(pattern)) {
                ...
                if (hasHardcodedText(value)) offenders.push(`${relative(file)}: ${match[0].trim()}`);
            }
        }
        expect(offenders).toEqual([]);
    });
...
describe('localization keys resolve', () => {
    ...
    test('every MEGS key used by a template or module is defined in English', () => {
    ...
    test('every MEGS key used by a template or module is translated to Portuguese', () => {
        const missing = systemKeys
    ...
    test('every key that takes a placeholder declares it in both dictionaries', () => {
